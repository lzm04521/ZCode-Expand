...
2. **写盘前必须过逐条自检**。重建产物要重新解析，26600+ 个未改动文件逐字节比对，改动文件的 `size` 与 `integrity` 与内容核对一致，才允许替换。
...
3. 在 `patches/<版本>/patches.json` 增加 target/edit：`find` / `replace` / `expect` / 可选 `marker`
   - 锚点尽量**包含后继字符**（如把 `,locale:` 一起写进 `find`），这样补丁天然幂等，重复 apply 不会重复插入
   - 需要幂等但不能靠锚点保证时，用 `marker` 声明"已应用的标志串"
